...
const critical = Effect.orDieWith(effect, (e) => new Error(`Critical failure: ${e.message}`));
```
...
```typescript
// Map error type
const mapped = Effect.mapError(effect, (e) => new HttpError(500, e.message));
...
const getUser = pipe(
   ...
   Effect.catchTag(
      "DbError",
      (e) => e.code === "NOT_FOUND" ? Effect.fail(new NotFoundError({ id })) : Effect.fail(new DatabaseError({ message: e.message })),
   ),
);
