...
const UserRepositoryTest = Layer.succeed(UserRepository, {
   findById: (id) => id === "existing-user" ? Effect.succeed({ id, name: "Test User", email: "test@example.com" }) : Effect.fail(new NotFoundError({ id })),
   save: () => Effect.void,
});
...
describe("Stream processing", () => {
   ...
   it("should handle stream errors", async () => {
      const input = Stream.fromIterable([1, 2, 3]).pipe(
         Stream.mapEffect((n) => n === 2 ? Effect.fail(new Error("boom")) : Effect.succeed(n)),
      );
