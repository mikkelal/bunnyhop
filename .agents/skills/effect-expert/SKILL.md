...
const program = pipe(
   Effect.succeed(5),
   Effect.map((n) => n * 2),
   Effect.flatMap((n) => n > 5 ? Effect.succeed(n) : Effect.fail(new Error("too small"))),
...
const findUser = (id: string): Effect.Effect<User, NotFoundError> =>
   pipe(
      db.query(id),
      Effect.flatMap((user) => user ? Effect.succeed(user) : Effect.fail(new NotFoundError({ id }))),
   );

// Defects - for bugs, not domain errors
const divide = (a: number, b: number): Effect.Effect<number> => b === 0 ? Effect.die(new Error("Division by zero - this is a bug!")) : Effect.succeed(a / b);
```
...
```typescript
const program = Effect.gen(function* () {
   yield* Effect.addFinalizer((exit) => Effect.log(`Cleanup: ${exit._tag}`));
   // ... do work
});
